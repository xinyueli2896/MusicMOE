    ...
    def forward(
        ...
        if emb_fn is not None:
            ...
            if adapter is not None:
                ...
                if mode is not None: # during inference
                    print("mode is ", mode)
                    dt_q = dt_q[:, :, interval[0]:interval[1], :]
                    prefix_k = prefix_k[:,:,interval[0]:interval[1],:]
                    prefix_v = prefix_v[:,:,interval[0]:interval[1],:]
                print("shapessss", dt_q.shape, query_layer.shape) # 为什么dt_q是75， q是76
                dt_q = dt_q + query_layer #TODO: 截断
                # dt_x = torch.nn.functional.scaled_dot_product_attention(
                #     dt_q, prefix_k, prefix_v, is_causal=False, dropout_p=p)
                # context_layer = emb_fn.modify(context_layer, dt_x, gate)
                attention_scores = torch.matmul(dt_q, prefix_k.transpose(-1, -2))
                ...
                context_layer_new = torch.matmul(attention_probs, prefix_v)
                if mode!='sanity': 
                    context_layer = emb_fn.modify(context_layer, context_layer_new, gate)
        ...
        if return_qkv:
            return query_layer, key_layer, value_layer, outputs
...
class RoFormerEncoder(nn.Module):
    ...
    def forward(
        ...
        if emb_fn is not None: emb_fn.update_adaptor(None)
        for i, layer_module in enumerate(self.layer):
            if emb_fn is not None: emb_fn.set_index(i)
            ...
            hidden_states = layer_outputs[0] # 为什么peft的是79， 正常的80？？？？
            # print("hidden_states", hidden_states.shape)
            # print(hidden_states)
            # hidden_states_log.append(hidden_states)
            if use_cache:
        ...
        if output_hidden_states:
            all_hidden_states = all_hidden_states + (hidden_states,)
            print("hidden_states", all_hidden_states[0].shape, all_hidden_states[1].shape)
        if not return_dict:
            return tuple(
                v
                for v in [
        ...
        # torch.save(hidden_states_log, "hidden_states.pth")
